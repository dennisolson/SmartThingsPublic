...
*/
		...
		standardTile("heatLevelUp", "device.heatingSetpoint", width: 3, height: 1, canChangeIcon: false, inactiveLabel: false, decoration: "flat") {
			state "heatLevelUp", label:'Heat', action:"heatLevelUp", icon:"st.thermostat.thermostat-up", backgroundColor: "#ffffff"
		}
		...
		standardTile("heatLevelDown", "device.heatingSetpoint", width: 3, height: 1, canChangeIcon: false, inactiveLabel: false, decoration: "flat") {
			state "heatLevelDown", label:'Heat', action:"heatLevelDown", icon:"st.thermostat.thermostat-down", backgroundColor: "#ffffff"

		}
		standardTile("coolLevelUp", "device.coolingSetpoint", width: 3, height: 1, canChangeIcon: false, inactiveLabel: false, decoration: "flat") {
			state "coolLevelUp", label:'Cool', action:"coolLevelUp", icon:"st.thermostat.thermostat-up",  backgroundColor: "#ffffff"
		}
		...
		standardTile("coolLevelDown", "device.coolingSetpoint", width: 3, height: 1, canChangeIcon: false, inactiveLabel: false, decoration: "flat") {
			state "coolLevelDown", label:'Cool', action:"coolLevelDown", icon:"st.thermostat.thermostat-down",backgroundColor: "#ffffff"
		}
		...
		standardTile("switchProgram", "device.programNameForUI",  canChangeIcon: false,
			...
			state "Home", label: 'Set Program Now: ${name}', action: "sleep", 
				icon: "st.Home.home4"
			...
			state "QuickSave", label: 'Set Program Now: ${name}', action: "home", 
				icon: "st.Home.home1"
			state "Custom", label: 'Set Program Now: ${name}', action: "resumeThisTstat", 
		...
		valueTile("fanMinOnTime", "device.fanMinOnTime", inactiveLabel: false,
			decoration: "flat", width: 3, height: 2) {
			state "default", label: 'FanMinOnTime\n${currentValue}'
		}
		// Program Tiles
		valueTile("programScheduleName", "device.programScheduleName", inactiveLabel:
			false, width: 3, height: 2, decoration: "flat") {
		...
		valueTile("weatherPop", "device.weatherPop", inactiveLabel: false, 
 			width: 3,height: 2, decoration: "flat") {
			state "default", label: 'Rain/Snow\nProbability\n${currentValue}%', unit: "%",
 			backgroundColor: "#ffffff"
		}
		...
		details(["thermostatMulti",
			...
			"equipStatus",
 			"alerts",
			"weatherDateTime", "weatherIcon",  
			"weatherConditions", "weatherTemperature", 
/* Commented out in new UI            
			"weatherTempHigh", "weatherTempLow", "weatherPressure",
...
def evaluate(temp, heatingSetpoint, coolingSetpoint) {
	...
	def threshold = (getTemperatureScale() == 'C') ? 0.5 : 1
	...
	if (mode in ["heat","emergency heat","auto"]) {
		if ((temp - heatingSetpoint) >= threshold) {
			...
			if (settings.trace) {
				log.debug "increasing heat to $temp"
			}	        
		}
			...
			setHeatingSetpoint(temp)
			if (settings.trace) {
				log.debug "decreasing heat to $temp"
			}	        
		}
	}
	if (mode in ["cool","auto"]) {
		if ((coolingSetpoint - temp) >= threshold) {
			...
			if (settings.trace) {
				log.debug "increasing cool to $temp"
			}	        
		} else if (((temp - coolingSetpoint) >= threshold) && (!heating)) {
			idle = true
			setCoolingSetpoint(temp)
			if (settings.trace) {
				log.debug "decreasing cool to $temp"
			}	        			
		}
	}
...
def setTemperature(value) {
	if (settings.trace) {
		log.debug "setTemperature>initial value= $value"
	}	        
	def mode = device.currentValue("thermostatMode")
	...
	if (getTemperatureScale() == 'C') { 		
		...
		if (settings.trace) {
			log.debug "setTemperature in Celsius>after temp= $temp vs. state.currentTileValue=$state.currentTileValue"
		}	        
		...
		state?.currentTileValue=temp	        
		if (settings.trace) {
			log.debug "setTemperature in Farenheits>after temp= $temp vs. state.currentTileValue=$state.currentTileValue"
		}	        
		evaluate(temp, heatTemp, coolTemp)
	}        
...
//		groups: (ecobeeType.toUpperCase() == 'REGISTERED')? getThermostatGroups(thermostatId) : 'No groups',
	...
	sendEvent name: "verboseTrace", displayed: ((settings.trace)?: false), value:
		"poll>done for thermostatId =${thermostatId}"
}
...
/*        
		if (!refresh_tokens()) {
			if ((exceptionCheck) && (state.exceptionCount >= MAX_EXCEPTION_COUNT) && (exceptionCheck.contains("Unauthorized"))) {
				log.error ("api>$exceptionCheck, not able to renew the refresh token; need to re-login to ecobee via MyEcobeeInit....")         
				sendEvent (name: "verboseTrace", value:"api>$exceptionCheck, not able to renew the refresh token;need to re-login to ecobee via MyEcobeeInit....")         
			}
		} else {
...
*/        
	...
	if (state.exceptionCount >= MAX_EXCEPTION_COUNT) {
		...
		sendEvent (name: "verboseTrace", displayed: ((settings.trace)?: false), value:
			"api>error: found a high number of exceptions (${state.exceptionCount}), last exceptionCheck=${exceptionCheck}, about to reset counter")         
...
//			sendEvent name: "verboseTrace", value: "doRequest>token= ${data.auth.access_token}"
		...
		if (settings.trace) {    
			log.debug "doRequest>trapped exception $e, and continue processing with refresh_tokens() call"
			sendEvent name: "verboseTrace", displayed: ((settings.trace)?: false), value:
				"doRequest>trapped exception $e,and continue processing with refresh_tokens() call"           
		}    
		refresh_tokens()        
...
void setThermostatSettings(thermostatId,tstatSettings = []) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('setThermostatSettings', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "setThermostatSettings>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"setThermostatSettings>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"setThermostatSettings>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
...
void setHoldExtraParams(thermostatId, coolingSetPoint, heatingSetPoint, fanMode,
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('setHold', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "setHold>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"setHold>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"setHold>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void createVacation(thermostatId, vacationName, targetCoolTemp, targetHeatTemp,
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('createVacation', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "createVacation>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"createVacation>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"createVacation>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void deleteVacation(thermostatId, vacationName) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('deleteVacation', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "deleteVacation>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verbose_trace", value:
						"deleteVacation>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"deleteVacation>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void resumeProgram(thermostatId=settings.thermostatId, resumeAllFlag=true) {  
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('resumeProgram', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "resumeProgram>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verbose_trace", value:
						"resumeProgram>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"resumeProgram>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
def getGroups(thermostatId) {
	...
	api('getGroups', bodyReq) {resp ->
		int statusCode = resp.data.status.code
		def message = resp.data.status.message
		if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
			if (settings.trace) {
				log.debug "getGroups>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
				sendEvent name: "verboseTrace", value:
					"getGroups>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
			}
			refresh_tokens()     
		}
		def exceptionCheck=device.currentValue("verboseTrace")
...
void updateGroup(groupRef, groupName, thermostatId, groupSettings = []) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('updateGroup', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "updateGroup>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"updateGroup>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"updateGroup>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void deleteGroup(groupRef, groupName) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('updateGroup', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "deleteGroup>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"deleteGroup>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"deleteGroup>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
	...
	for (i in 0..data.thermostatList.size() - 1) {
		...
		while ((statusCode) && (j++ <2)) { // retries once if api call fails
			...
			api('setHold', bodyReq) {resp ->
				...
				if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
					if (settings.trace) {
						log.debug "setClimate>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
						sendEvent name: "verboseTrace", value:
							"setClimate>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
					}
					refresh_tokens()     
				}
				exceptionCheck=device.currentValue("verboseTrace")
			...
			if (exceptionCheck?.contains("exception")) {
				log.error
					"setClimate>exception=${exceptionCheck.toString()}, loop counter=$j"
				state.exceptionCount = state.exceptionCount +1     
				statusCode=(statusCode)?:3 //internal error            
				continue
			}                
...
//		call getThermostatInfo if a value for thermostatId is provided to make sure to have the right thermostat information
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		api('updateClimate', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "updateClimate>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"updateClimate>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"updateClimate>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void controlPlug(thermostatId, plugName, plugState, plugSettings = []) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('controlPlug', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "controlPlug>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"controlPlug>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"controlPlug>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void getReportData(thermostatId, startDateTime, endDateTime, startInterval, endInterval, reportColumn, includeSensorData, postData='false') {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('runtimeReport', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "getReportData>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"getReportData>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
			if (!statusCode) {
				...
				sendEvent name: "verboseTrace", displayed: ((settings.trace)?: false)
                	value:"getReportData>done for thermostatId ${thermostatId}"
				...
				sendEvent name: "verboseTrace", value:
					"getReportData>error=${statusCode},message= ${message} for ${thermostatId}"			} /* end if statusCode */
		} /* end api call */                
		if (exceptionCheck?.contains("exception")) {
			log.error
				"getReportData>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void getRemoteSensorUpdate(thermostatId=settings.thermostatId) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('thermostatInfo', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "getRemoteSensorUpdate>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"getRemoteSensorUpdate>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"getRemoteSensorUpdate>exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void getThermostatInfo(thermostatId=settings.thermostatId) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
		...
		api('thermostatInfo', bodyReq) {resp ->
			...
			if ((resp.status == 500) && (statusCode == ECOBEE_NEED_TOKEN_REFRESH)) {
				if (settings.trace) {
					log.debug "getThermostatInfo>thermostatId=${thermostatId}, error $statusCode, need to call refresh_tokens()"
					sendEvent name: "verboseTrace", value:
						"getThermostatInfo>thermostatId=${thermostatId},error $statusCode, need to call refresh_tokens()"
				}
				refresh_tokens()     
			}
			exceptionCheck=device.currentValue("verboseTrace")
			if (!statusCode) {
				...
				sendEvent name: "verboseTrace", value:
					"getTstatInfo>error=${statusCode},message=${message} for ${thermostatId}"			} /* end if statusCode */                 
		} /* end api call */
		if (exceptionCheck?.contains("exception")) {
			log.error
				"getThermostatInfo> exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
void getThermostatSummary(tstatType) {
	...
	while ((statusCode) && (j++ <2)) { // retries once if api call fails
			...
			if (!statusCode) {
				...
				for (i in 0..data.thermostatCount - 1) {
					...
					if (settings.trace) {
						...
						log.debug "getThermostatSummary>intervalRevision=${intervalRevision},runtimeRevision=${runtimeRevision},thermostatRevision=${thermostatRevision}" 
						sendEvent name: "verboseTrace", value:
		...
		if (exceptionCheck?.contains("exception")) {
			log.error
				"getThermostatSummary> exception=${exceptionCheck.toString()}, loop counter=$j"
			state.exceptionCount = state.exceptionCount +1     
			statusCode=(statusCode)?:3 //internal error            
			continue
		}                
...
def refreshLocalAuthToken() {	
	boolean refresh_success=false
	def REFRESH_SUCCESS_CODE=200    
    
    
	if (settings.trace) {                        
		log.debug "refreshLocalAuthToken>about to refresh auth token"
	}
	...
	if (settings.trace) {                        
		log.debug "refreshLocalAuthToken>refreshParams=$refreshParams"
	}        
	def jsonMap
	httpPost(refreshParams) { resp ->
		...
		if (resp.status == REFRESH_SUCCESS_CODE) {
			if (settings.trace) {                        
				log.debug "refreshLocalAuthToken>token refresh done resp = ${resp.data}"
			}
			jsonMap = resp.data
			...
			if (resp.data) {
				...
				if (settings.trace) {                        
					log.debug("refreshLocalAuthToken>new authexptime = ${authexptime}")
					log.debug("refreshLocalAuthToken>new access token = ${data.auth.access_token}")
					log.debug("refreshLocalAuthToken>new refresh token = ${data.auth.refresh_token}")
				}  
				refresh_success=true   
			} /* end if resp.data */
		} else {
			log.error "refreshLocalAuthToken>refresh failed ${resp.status} : ${resp.status.code}"
...
private def refresh_tokens() {
	...
	if ((state?.lastRefreshTimestamp) && (state?.lastRefreshTimestamp > time_check_for_refresh)) {
		if (settings.trace) {
			log.debug "refresh_tokens>time_check_for_refresh (${time_check_for_refresh}) < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
			sendEvent name: "verboseTrace", value:
				"refresh_tokens>time_check_for_refresh (${time_check_for_refresh} < state.lastRefreshTimestamp (${state?.lastRefreshTimestamp}), not refreshing tokens..."
		}
		return false
	}
...
//				log.warn "refresh_tokens>local tokens still expired after refreshParentTokens() call"
//			}
//		}            
	...
	if (!isTokenExpired()) {    
		...
		if (settings.trace) {
			...
			log.warn "refresh_tokens>local tokens still expired after refresh_tokens() call"
		}
		return false
	}    
}
...
private def refreshChildTokens(auth) {
	if (settings.trace) {
		log.debug "refreshChildTokens>begin new token auth= $auth"
		log.debug "refreshChildTokens>old data.auth= $data.auth"
	}
	if (auth.authexptime>data.auth.authexptime) {    
		...
		if (settings.trace) {
			log.debug "refreshChildTokens>end newly refreshed data.auth=$data.auth"
		}
		return true        
	}        
	return false    
...
private void login() {
	...
	if (data?.auth?.thermostatId) {
		...
		if (settings.trace) {
			log.error "login>should be already logged in..."
		}
	} else { 
		setAuthTokens()
	}    
...
private def isTokenExpired() {
	...
	if (settings.trace) {
		log.debug "isTokenExpired>expiresIn timestamp: ${data.auth.authexptime} > timestamp check for exp: ${time_check_for_exp}?"
		log.debug "isTokenExpired>expires in ${authExpTimeInMin.intValue()} minutes"
		log.debug "isTokenExpired>data.auth= $data.auth"
		sendEvent name: "verboseTrace", value: "isTokenExpired>data.auth= $data.auth"
		sendEvent name: "verboseTrace", value:
			"isTokenExpired>expires in ${authExpTimeInMin.intValue()} minutes"
	}            
    if (authExpTimeInMin <0) {
		if (settings.trace) {
 			log.warn "isTokenExpired>auth token buffer time expired (${buffer_time_expiration} min.), countdown is ${authExpTimeInMin.intValue()} minutes, need to refresh tokens now!"
			sendEvent name: "verboseTrace", value:
				"isTokenExpired>auth token buffer time  expired (${buffer_time_expiration} min.), countdown is ${authExpTimeInMin.intValue()} minutes, need to refresh tokens now!"
		}
	}    
    if (authExpTimeInMin < (0-buffer_time_expiration)) {
		if (settings.trace) {
			log.warn "isTokenExpired>refreshing tokens is more at risk (${authExpTimeInMin} min.),exception count may increase if tokens not refreshed!"
			sendEvent name: "verboseTrace", value:
				"isTokenExpired>refreshing tokens is more at risk (${authExpTimeInMin} min.),exception count may increase if tokens not refreshed!"
		}                
	}    
